...
class CodeView(Text):
    _w: str
    _builtin_color_schemes = {"ayu-dark", "ayu-light", "dracula", "hacker", "mariana", "monokai"}
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._frame.grid(row=0, column=0, sticky="nsew")  # Fill the entire window
        ...
        self._vs = Scrollbar(
            self._frame, autohide=autohide_scrollbar, orient="vertical", command=self.yview
        )
        self._hs = Scrollbar(
        ...
        self._hs.grid(row=1, column=0, columnspan=3, sticky="we")

        super().configure(
            yscrollcommand=self.vertical_scroll,
            xscrollcommand=self.horizontal_scroll,
            tabs=Font(font=kwargs["font"]).measure(" " * 1),
        )
